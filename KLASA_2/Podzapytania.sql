...
SELECT klienci.NazwiskoKlienta, klienci.ImieKlienta, zamowienia.DataZamowienia, zamowienia.DataWysylki 
FROM klienci INNER join zamowienia ON klienci.IDKlienta=zamowienia.IDKlienta 
WHERE zamowienia.DataWysylki LIKE '2017-10-03'
 
-- 2. Wymień wszystkich klientów i liczbę zamówień złożonych przez każdego z nich.
SELECT klienci.ImieKlienta, klienci.NazwiskoKlienta, COUNT(zamowienia.NumerZamowienia) AS Liczbazamowien 
FROM klienci INNER join zamowienia ON klienci.IDKlienta=zamowienia.IDKlienta
...
SELECT klienci.ImieKlienta, klienci.NazwiskoKlienta, zamowienia.NumerZamowienia, zamowienia.DataZamowienia, zamowienia.DataWysylki,
	(SELECT MAX(zamowienia.DataZamowienia)
	FROM zamowienia 
	WHERE klienci.IDKlienta=zamowienia.IDKlienta) AS DataOstZam
FROM klienci INNER join zamowienia ON klienci.IDKlienta=zamowienia.IDKlienta 
